...
const isExistLecturerInGroup = async (req, res, next) => {
    ...
    const query =
        'select l.id from lecturers l join lecturer_terms lt on lt.lecturer_id = l.id join group_lecturer_members glm on glm.lecturer_term_id = lt.id where glm.group_lecturer_id = :id and l.id = :lecturerId';
...
const quantityOfGroup = async (req, res, next) => {
    ...
    const members = GroupLecturerMember.findAll({
        where: {
